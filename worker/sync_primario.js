    ...
    try {
      ...
      for (const { col, dia } of colsDias) {
        ...
        acc.presentismo.push({
          dni, fecha, nivel: 'PRIMARIO', turno, division, curso, estado,
          fuente_planilla: 'PRIMARIO',
        });
      }
    } catch (err) {
      stats.errores++;
...
async function syncPrimario() {
  ...
  try {
    ...
    await batchUpsertInternosDetalle(acc.detalle, 'PRIMARIO');
    stats.filas_insertadas = await batchInsertPresentismo(acc.presentismo);
  } catch (err) {
    stats.errores++;
